"""
Arbeitnow tech/AI jobs crawler.

Fetches the Arbeitnow job board API, validates 24-hour freshness,
...
def _infer_role_family(title: str, tags: list[str]) -> Optional[str]:
    ...
    if any(w in combined for w in ["engineer", "developer", "software", "machine learning", "ml", "ai", "backend", "frontend", "fullstack", "devops"]):
        return "Engineering"
    if any(w in combined for w in ["data scientist", "data analyst", "data"]):
        return "Data"
    if any(w in combined for w in ["product", "product manager"]):
        return "Product"
    if any(w in combined for w in ["designer", "ui", "ux", "design"]):
        return "Design"
    if any(w in combined for w in ["sales", "account executive", "account manager"]):
        return "Sales"
    if any(w in combined for w in ["marketing", "social media", "growth", "content"]):
        return "Marketing"
    return None
...
async def scrape_arbeitnow(
    ...
    """
    Scrape fresh tech/AI jobs from Arbeitnow.

    Returns:
        Dict with metrics: raw_fetched, survived_24h, saved.
    """
    logger.info("Fetching Arbeitnow jobs from %s", API_URL)
    raw_fetched = 0
    survived_24h = 0
    saved = 0
    ...
    try:
        ...
        logger.error("Failed to fetch Arbeitnow jobs: %s", exc)
        return {"raw_fetched": 0, "survived_24h": 0, "saved": 0}

    logger.info("Discovered %d raw jobs from Arbeitnow API", len(jobs))
    ...
    for job in jobs:
        ...
        if not is_fresh or not norm_date:
            logger.debug("[Arbeitnow] Stale job (>24h or missing date): '%s' at '%s' (%s)", title[:30], company, raw_date)
            continue
        ...
        if did_save:
            saved += 1
            logger.info(
                "[Arbeitnow #%d] Saved: %s at %s (date: %s, role: %s, remote: %s)",
                saved, title[:40], company, norm_date, role_family, is_remote
            )

    logger.info(
        "Arbeitnow complete: fetched %d raw, %d survived 24h filter, %d saved",
