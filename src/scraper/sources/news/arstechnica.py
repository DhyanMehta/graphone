"""
Ars Technica AI/Tech news crawler.

Fetches the Ars Technica main feed, filters for AI and frontier technology stories,
evaluates 24-hour publication freshness, crawls full-text article content,
...
AI_KEYWORDS = {
    ...
    "anthropic", "deepmind", "generative ai", "neural", "robotics", "model",
    "chip", "nvidia", "hardware", "tech", "computing"
}


async def scrape_arstechnica(
    client: HttpClient,
    db: aiosqlite.Connection,
    limit: Optional[int] = 30,
) -> dict:
    """
    Scrape fresh AI/Tech news articles from Ars Technica.

    Returns:
        Dict with metrics: raw_fetched, survived_24h, saved.
    """
    logger.info("Fetching Ars Technica feed from %s", FEED_URL)
    raw_fetched = 0
    survived_24h = 0
    saved = 0
    ...
    try:
        ...
        logger.error("Failed to fetch/parse Ars Technica feed: %s", exc)
        return {"raw_fetched": 0, "survived_24h": 0, "saved": 0}

    logger.info("Discovered %d raw items in Ars Technica feed", len(items))
    ...
    for item in items:
        ...
        # Relevance filter: title or categories match AI/frontier tech
        text_for_relevance = f"{title.lower()} {' '.join(categories)}"
        if not any(k in text_for_relevance for k in AI_KEYWORDS):
            continue
        ...
        if not is_fresh or not norm_date:
            logger.debug("[Ars Technica] Stale article (>24h or missing date): '%s' (%s)", title[:40], raw_date)
            continue
        ...
        try:
            ...
            content_div = (
                art_soup.find("div", class_="article-content")
                or art_soup.find("div", itemprop="articleBody")
            ...
            logger.warning("[Ars Technica] Could not fetch full text for '%s': %s", url, exc)
            content_type = "SUMMARY"
            full_text = summary or title
        ...
        if did_save:
            saved += 1
            logger.info(
                "[Ars Technica #%d] Saved: %s (date: %s, type: %s, len: %d)",
                saved, title[:50], norm_date, content_type, len(full_text or "")
            )

    logger.info(
        "Ars Technica complete: fetched %d raw, %d survived 24h filter, %d saved",
