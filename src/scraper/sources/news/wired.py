"""
Wired AI news crawler.

Fetches the Wired AI tag RSS feed, evaluates 24-hour publication freshness,
...
async def scrape_wired(
    client: HttpClient,
    db: aiosqlite.Connection,
    limit: Optional[int] = 30,
) -> dict:
    """
    Scrape fresh AI news articles from Wired.

    Returns:
        Dict with metrics: raw_fetched, survived_24h, saved.
    """
    logger.info("Fetching Wired AI feed from %s", FEED_URL)
    raw_fetched = 0
    survived_24h = 0
    saved = 0
    ...
    try:
        ...
        logger.error("Failed to fetch/parse Wired feed: %s", exc)
        return {"raw_fetched": 0, "survived_24h": 0, "saved": 0}

    logger.info("Discovered %d raw items in Wired feed", len(items))
    ...
    for item in items:
        ...
        if not is_fresh or not norm_date:
            logger.debug("[Wired] Stale article (>24h or missing date): '%s' (%s)", title[:40], raw_date)
            continue
        ...
        try:
            ...
            content_div = (
                art_soup.find("div", attrs={"data-testid": "BodyWrapper"})
                or art_soup.find("div", class_=lambda c: c and "body__inner-container" in c)
            ...
            logger.warning("[Wired] Could not fetch full text for '%s': %s", url, exc)
            content_type = "SUMMARY"
            full_text = summary or title
        ...
        if did_save:
            saved += 1
            logger.info(
                "[Wired #%d] Saved: %s (date: %s, type: %s, len: %d)",
                saved, title[:50], norm_date, content_type, len(full_text or "")
            )

    logger.info(
        "Wired complete: fetched %d raw, %d survived 24h filter, %d saved",
